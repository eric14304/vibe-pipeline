// Ticket Drawer — multi-state (StepDone / IterRunning / IterPaused / BranchMismatch)
const { useState: useS, useEffect: useEf, useRef: useR } = React;

/* ─────────────────────── shared atoms ─────────────────────── */

function DCaret({ open }) {
  return (
...
function ExpandIcon() {
  return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14v6h6M20 10V4h-6M14 4l6 6M10 20l-6-6" /></svg>;
}
function FileIcon() {
...
function DrawerIterRunning({ tick = 0 }) {
  const elapsed = 134 + tick; // seconds in current iter
...
function DrawerStepRunning({ tick = 0 }) {
...
function DrawerStage({ state = "iter-running", showSwitcher = true }) {
  ...
  return (
    <div className="drawer-stage">
      ...
      <div className="drawer-bg">
        {typeof Board !== "undefined" ? <Board density="medium" /> : null}
      </div>
...
Object.assign(window, {
  DrawerStage, DrawerStepDone, DrawerStepRunning, DrawerIterRunning, DrawerIterDone,
  DRAWER_STATES
});