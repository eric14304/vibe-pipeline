...
跑 [pause-simplify](archive/pause-simplify-2026-05-17.md) pipeline 前置 + 跑 t1/t2 時連續踩到的 robustness / UX bug 整理。獨立列,因為跟 pause-simplify 本身 spec 無關(spec 是「簡化 pause」,這些 bug 是 VP 自己各層的 robustness gap)。
...
**狀態**:已寫進 [pause-simplify-2026-05-17.md](archive/pause-simplify-2026-05-17.md) §t1 B 方案(`stopImmediate` 偵測無 entry → 拒絕 + 回 `repl_external` 錯誤)。
