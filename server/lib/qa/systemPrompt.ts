...
**禁止**創 `scope` / `loop` / `deliverable` / `format` / `commitStrategy` / 任何其他欄位。技術細節該寫進 `prompt` 裡,驗收條件該寫進 `acceptance` 裡。例:user 說「每輪修完一類就 commit」 → 把這條塞進 `prompt` 字串裡作為做法提示,**不是**新增 `commitStrategy` 欄位。
