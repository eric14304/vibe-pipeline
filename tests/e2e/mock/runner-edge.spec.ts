...
test("running 中按 Run → 後端 state guard 擋(409 / 4xx)", async ({ page, request }) => {
  ...
  // 第一次 Run
  const res1 = await request.post(`http://127.0.0.1:3003/api/projects/${proj.hash}/pipelines/p-edge/run`);
  expect(res1.ok()).toBe(true);

  // 第二次立即 Run → 應該被 guard 擋
  const res2 = await request.post(`http://127.0.0.1:3003/api/projects/${proj.hash}/pipelines/p-edge/run`);
...
test("merged pipeline 不准 Run → state guard 擋", async ({ request }) => {
  ...
  const res = await request.post(`http://127.0.0.1:3003/api/projects/${proj.hash}/pipelines/p-edge/run`);
