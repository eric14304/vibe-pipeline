...
export type QAVariant = "chat" | "form" | "step" | "drawer";

type QuickReply = { label: string; value: string | null; action?: string };

type AISuggestion = { head: string; items: string[] };
...
type Turn = {
  ...
  draftAfter?: Partial<Draft>;
  highlight?: (keyof Draft)[];
...
type HighlightMap = Partial<Record<keyof Draft, number>>;
...
export type QAScreenProps = {
  variant?: QAVariant;
  autoplay?: boolean;
  density?: "compact" | "medium";
};

export function QAScreen({ variant = "chat" }: QAScreenProps) {
  const [turnIdx, setTurnIdx] = useState(0);
  const [history, setHistory] = useState<ChatMsg[]>([]);
  const [draft, setDraft] = useState<Draft>({});
  const [highlight, setHighlight] = useState<HighlightMap>({});
  ...
  function answer(text: string | null, action?: string) {
    if (text == null) return;
  ...
  const currentTurn: Turn | undefined = TURNS[turnIdx];
  const lastAi = history.length > 0 && history[history.length - 1]!.from === "ai";
          ...
          <div className="qa-draft-list">
            {FIELD_DEFS.map((def) => (
              <DraftField
                ...
                def={def}
                value={draft[def.key]}
                pair={def.isPair && def.pairKey ? (draft[def.pairKey] as string | undefined) ?? null : null}
          ...
          <div className="qa-draft-foot">
            ...
            <div className="qa-draft-actions">
              ...
              <button
                ...
                disabled={!draft.title || !draft.goal || !draft.acceptance}
...
function SuggestionCard({ sug }: { sug: AISuggestion }) {
  return (
    <div
      style={{
...
type DraftFieldProps = {
  ...
  pair: string | null;
  justFilled: number | undefined;
};

function DraftField({ def, value, pair, justFilled }: DraftFieldProps) {
  ...
  return (
    <div className={"qa-field" + (!hasValue ? " is-empty" : "") + (isJust ? " is-just-filled" : "")}>
      ...
      {!hasValue ? (
      ...
      ) : def.isList && Array.isArray(value) ? (
        <ul className="qa-field-list">
          {value.map((v, i) => (
            <li key={i}>{v}</li>
          ))}
        </ul>
      ) : def.isMode && typeof value === "string" ? (
        ...
        <div className="qa-field-value mono" style={{ fontSize: 12, lineHeight: 1.6 }}>
          <div><span style={{ color: "var(--fg-mute)" }}>執行者</span> · <span style={{ color: "var(--fg)" }}>{value as string}</span></div>
          <div style={{ color: "var(--fg-mute)" }}>審核者 · <span style={{ color: pair && pair !== "—" ? "var(--fg)" : "var(--fg-faint)" }}>{pair || "—"}</span></div>
        </div>
      ) : (
        <div className="qa-field-value">{value as string}</div>
      )}
    </div>
  );
}

function DraftProgress({ draft }: { draft: Draft }) {
...
type FormVariantProps = {
  ...
  answer: (text: string | null, action?: string) => void;
  aiTyping: boolean;
  canShowQR: boolean;
  savedDone: boolean;
};

function FormVariant({ draft, highlight, currentTurn, answer, aiTyping, canShowQR, savedDone }: FormVariantProps) {
  return (
    <>
...
function FormField({ label, value, highlight, placeholder, multi, rows = 3, mono }: FormFieldProps) {
...
type StepVariantProps = {
  turnIdx: number;
  answer: (text: string | null, action?: string) => void;
  draft: Draft;
  composerVal: string;
  setComposerVal: (v: string) => void;
  savedDone: boolean;
};

function StepVariant({ turnIdx, answer, draft, composerVal, setComposerVal, savedDone }: StepVariantProps) {
  ...
  if (savedDone)
    return (
      <div className="qa-step-shell">
        <div className="qa-step-card" style={{ alignItems: "center", textAlign: "center", justifyContent: "center" } as CSSProperties}>
      ...
      <aside className="qa-drawer">
        ...
        <div className={"qa-drawer-draft" + (draftOpen ? " is-open" : "")}>
          <button className="qa-drawer-draft-head" onClick={() => setDraftOpen((o) => !o)}>
            ...
            <svg
              ...
              strokeWidth="2"
              strokeLinecap="round"
              style={{ transform: draftOpen ? "rotate(180deg)" : "none", transition: "transform 200ms" }}
            >
              <path d="m6 9 6 6 6-6" />
            </svg>
          </button>
          {draftOpen && (
            <div className="qa-drawer-draft-body">
              {FIELD_DEFS.map((def) => {
                ...
                const hl = highlight[def.key];
                const isJust = !!hl && Date.now() - hl < 1500;
                return (
        ...
        <div className="qa-drawer-chat" ref={scrollRef}>
          ...
          {savedDone && (
            <div className="qa-msg from-ai">
              <span
                className="qa-msg-avatar"
                style={{ background: "var(--done-soft)", color: "var(--done)", borderColor: "color-mix(in srgb, var(--done) 30%, transparent)" }}
              >
                ✓
              </span>
              <div
                className="qa-msg-bubble"
                style={{ background: "var(--done-soft)", borderColor: "color-mix(in srgb, var(--done) 30%, transparent)" }}
              >
                Ticket #6 已建立 · 開始排程 doer。<br />
        ...
        <Composer value={composerVal} onChange={setComposerVal} onSubmit={onSubmit} placeholder={composerPh} disabled={savedDone} />
      </aside>
    </>
  );
}
...
function DollarIcon() {
  return (
    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 2v20M17 6.5C17 4.6 14.8 3 12 3S7 4.6 7 6.5 9.2 10 12 10s5 1.6 5 3.5-2.2 3.5-5 3.5-5-1.6-5-3.5" /></svg>
  );
}
