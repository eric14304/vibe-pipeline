...
**`server/lib/pipelineMerge.ts:autoMergeNoAI()`**(2026-05-13 新)— Auto-merge 走機械式 git,不 spawn AI
- `autoMerge=true` 觸發時 `orchestrator.maybeAutoMerge()` 改呼這條(原本走 `triggerMerge` 開 AI runner,慢 1-3min + 每次燒 30k-100k token)
- 純 git CLI:checkout base → `git merge --no-ff <pipelineBranch>` → success 寫 pipeline.mergeCommit + state=merged,毫秒級
- 衝突 → `git merge --abort` + emit `merge_blocked` notif("X 衝突檔,需手動 AI 解"),**不自動 spawn AI**(user 主動點 manual merge 才走 AI)
- 心智:autoMerge 是「便利開關」(無風險 happy path 自動推進),不是「AI 全自動代勞」;燒 token 決策回到 user
- Manual `/merge` 路徑(`triggerMerge`)維持現狀走 AI(user 主動點 = 接受 AI 服務 + 等待)
...
**`server/lib/pipelineDir.ts`** — init idempotent
