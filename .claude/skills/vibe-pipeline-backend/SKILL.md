...
**`server/lib/pipelineMerge.ts:autoMergeNoAI()`**(2026-05-13 新)— Auto-merge 二段式
...
- 第 2 段:`orchestrator.maybeAutoMerge` 看 reason 分流
  - conflict → **自動升級** call `triggerMerge`(spawn AI 全套,同 manual 路徑),emit `pipeline_auto_merge_started` 紀錄升級事件
  - dirty / git_error / not_found / running → 不適合 AI 自動解,emit `merge_blocked` 等 user
- 心智:autoMerge=true 是「全自動」承諾;clean case ~90% 秒結束,conflict case 跟過去一樣慢(1-3min)但無人值守
- Manual `/merge`(user 主動點)直接走 `triggerMerge` AI 全套(不經 autoMergeNoAI)
