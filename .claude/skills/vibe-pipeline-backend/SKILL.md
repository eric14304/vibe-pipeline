...
description: vibe-pipeline 後端 / 執行層規格與計畫 — 目前 0 行 backend 程式碼,本 SKILL 是 spec 索引與架構記憶。實作 doer/critic runner、SQLite log、git branch / worktree、notification 通道、SKILL 蒸餾、Q&A 收斂引擎、budget tracker、`vp` CLI 之前先讀。
---
...
- 不裝 ORM(這版只寫 YAML,不碰 SQLite)
- YAML 解析:`yaml` 套件
- Schema 驗證:**這版不做**(直接信任 YAML 結構,壞了拋 error)
...
- **`~/.vibe-pipeline/state.json`** 只存 global runtime(user 開過的 projects + last selected)。**不存 pipeline / ticket 細節**,那些都從各 project 的 `.tickets/` 內讀
...
GET  /api/projects/:hash/status              回 {hasTickets, name, path}
POST /api/projects/:hash/init                建 .tickets/ 結構

GET  /api/projects/:hash/pipelines           掃 .tickets/pipelines/*.yaml
POST /api/projects/:hash/pipelines           body {name, baseBranch, mergeStrategy} → 寫 YAML
GET  /api/projects/:hash/pipelines/:id       單條

GET  /api/projects/:hash/tickets             掃 .tickets/tickets/*.yaml
POST /api/projects/:hash/tickets             body {pipelineId, n, title, mode} → 寫 YAML
GET  /api/projects/:hash/tickets/:id         單張
```

### Response envelope

統一 `{ ok: true, data }` 或 `{ ok: false, error: { code, message } }`。
常見 error code:`not_found` / `permission_denied` / `dialog_cancelled` / `invalid_path` / `tickets_not_initialized` / `already_initialized`。
...
`.tickets/` 結構樹 → root [`CLAUDE.md`](../../../CLAUDE.md) § Repo 外。本段只寫規則:
- `.tickets/` 是 **target repo 根目錄底下**(不是 vibe-pipeline 自己的 repo),由 `vp init` 建
- **git 規則**:`vp init` 寫 `.tickets/.runtime/` 到該 repo 的 `.gitignore`
- `tickets/` `pipelines/` `skills/` 都 git tracked,是 **single source of truth**(信條 #1);`.runtime/` 是 SQLite cache,可重建
- SKILL 蒸餾走 `.candidates/` stage,不直寫 `SKILL.md`(信條 #3)
...
`[P2]` Worktree 模式:pipeline 設 `isolated: true` → `git worktree add .tickets/.worktrees/{slug}`,完成 `git worktree remove`。失敗時 worktree 留著供人上去看。
...
- `[P2]` Pipeline 完成 → critic 蒸餾出 summary,寫回 pipeline YAML 的 `summary` 欄位
- `[P3]` 跨 pipeline 重複出現的模式 → AI 提議 SKILL 候選,進 `.tickets/skills/.candidates/`(git tracked,等使用者審)
- `[P3]` `vp skill review` 列 pending、走 Q&A 引導措辭、approve 後 commit 到 `SKILL.md`
- **AI 永遠不直接寫 SKILL.md**(設計信條 #3)
- 跨 pipeline 不直傳 context(信條 #4),走 SKILL 中介
...
| Daily / project | `config.yaml budget.daily` | 通知 | 當前 ticket 跑完才停 |

**Hard cap 不砍中途**(信條延伸:不損毀 SQLite 一致性)。Pricing table 在 `config.yaml budget.pricing`,使用者可改。
...
| 寫檔給其他工具讀 | `[P2]` | 同上,可選(`.tickets/.runtime/notifications/*.json`) |
...
- 提到 `vp` CLI、`.tickets/`、SQLite、git branch/worktree、doer/critic、SKILL 蒸餾、Q&A 引擎、budget、notification、stall detection、intervention 任何一項
- 開工新 backend 模組
