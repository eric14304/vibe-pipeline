...
**進入串接期**(2026-05-09)。前一階段完成 6 個 pixel-perfect 畫面 + 純 mock data,**所有 action 是 stub**。

第一條 vertical slice:**空狀態 → 選資料夾 → init popup → 後端紀錄**。詳見主 SKILL 「當前 phase」段與 `refs/integration-plan-v1-2026-05-09.md`。

進串接後加新東西的優先序:
1. **Backend API 串接**(取代 `src/data/*` 的 const seed)
2. **空狀態畫面**(沒選 project / 有 project 沒 ticket / 沒 pipeline 等)
3. **跨畫面 navigation + state**(active project 全 app 共用)
4. **InitPopup**(新元件,沒 prototype 對照)

別在串接期繼續加新「demo data 滿的」變體 — 那會浪費。
