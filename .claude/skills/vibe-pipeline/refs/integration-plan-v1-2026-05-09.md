...
3. backend 偵測該資料夾 .tickets/ 是否存在
   ├─ 沒有 → 彈 InitPopup 詢問是否自動初始化
   │   └─ user 同意 → backend 跑 init (建 .tickets/ + 寫 .gitignore + 預設 config.yaml)
   │       └─ 完成 → InitPopup 關 → 切回空 board (此時 已 ready 加 ticket)
   └─ 有 → 直接載入 (此時讀 .tickets/ 內現有 ticket / pipeline 顯示)
4. 空 board 上點「+ 新 pipeline」 → CreateCard → 寫進 .tickets/pipelines/<name>.yaml
5. 進到該 pipeline → 「+ ticket」 → 寫進 .tickets/tickets/<id>.yaml
```
...
### 3.1 Project 模組
- `Project { path, name, hasTickets, lastOpenedAt }`
...
- API:
  ...
  - `GET /api/projects/:hash/status` — 該 project 的 .tickets/ 偵測結果

### 3.2 Init 模組
- `POST /api/projects/:hash/init` — 在指定 project 建 .tickets/ 結構
  - 建 `.tickets/{config.yaml, tickets/, pipelines/, skills/}`
  - 寫 `.tickets/.runtime/` 到 `.gitignore`(若不存在)
  - 預設 `config.yaml` 用簡化版(只填 `defaults.base_branch: main`、其他留空)

### 3.3 Pipeline / Ticket CRUD(YAML 層)
...
- 有 project + 沒 .tickets/ → 渲染 InitPopup
- 有 project + 有 .tickets/ → 載 pipelines(目前空就空著)
...
- **有 project + 沒 .tickets/**: Modal InitPopup 蓋上,Board 在背後 dim
- **有 project + 有 .tickets/ 但沒 pipeline**: Rail 只有「+ 新 pipeline」按鈕,Focus 放 empty illustration「**還沒任何 pipeline,點左邊建立第一條**」
...
- **vibe-kanban**:
  ...
  - **Repository lifecycle script**:這版不做,但 config.yaml 預留 `setup / dev / cleanup` script 欄
- **Symphony WORKFLOW.md 模式**:這版不採用(我們是分散 YAML),但**注意保留可走那條路的彈性** — 別把 `pipelines/*.yaml` 寫死成只能機器讀,之後 markdown body 要能放 prompt template
...
- **檔案監聽**:user 從 IDE 直接編輯 `.tickets/*.yaml`,UI 要不要即時更新?第一版**不做**(refresh 才更新),之後可加 `chokidar` watcher
- **CORS / 認證**:本機 server 跑在 localhost:3001,前端 5173。Vite proxy `/api/*` 過去就好,不需要 CORS。**沒做認證**(本機,信任 user)。
- **Init 後是否要 `git init`** 若該資料夾沒 git?第一版**不做**,init 只建 .tickets/,git init 留給 user
