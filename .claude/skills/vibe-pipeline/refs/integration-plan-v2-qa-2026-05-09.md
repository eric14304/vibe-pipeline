...
產出可給 doer agent 直接執行的 ticket spec。
...
```json
{
  ...
  "spec": {
    ...
    "goal": "...",
    "acceptance": ["...", "..."],
    "prompt": "doer agent 接到這段就能開工的完整 prompt",
    "mode": "iter"
  }
}
```
...
- 多輪以 3-6 輪為目標,不囉嗦
- mode 預設 iter (有 critic 迴圈),簡單一次性事務才用 step
```
...
- 真的 doer / critic runner 跑 ticket(QA 只負責「建立」,不負責「執行」)
